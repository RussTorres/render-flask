...
def process_group(render,matchcollections,group):
    matches = []
    #matchcolllection = matchcollections.pop()
    
    #for matchcollection in matchcollections:
    matches=renderapi.pointmatch.get_matches_outside_group(matchcollections[0],
                                                        group,
                                                        mergeCollections=matchcollections[1:],
                                                        render=render)
    #print matches
    matchcounts = {}
...
def get_groups(render,matchcollection):
    groups=renderapi.pointmatch.get_match_groupIds(matchcollection,render=render)
    ...
    for answer,group in zip(answers,groups):
        ...
        for group2 in answer.keys():
            if group2 not in zvalues.keys():
                z = renderapi.stack.get_section_z_value(stack,group2,render=render)
...
def make_pointmatch_summary_plot(render,stack,matchcollection,sections_per_row=200):
    ...
    for row in range(rows):
        ...
        for z in first_section_zs:
            if (z>=startz)&(z<=endz):
                theax.plot([z-.5,z-.5],[-maxdz,maxdz],c='w',linewidth=2,linestyle='--',alpha=.5)
    return f
...
def make_pointmatch_plot(r,stack,matchcollection,z_p,z_q,max_d=40):
    ...
    allmatches = renderapi.pointmatch.get_matches_from_group_to_group(matchcollection,section_p,section_q,render=r)
    tilespecs_p = renderapi.tilespec.get_tile_specs_from_z(stack,z_p,render=r)
    tilespecs_q = renderapi.tilespec.get_tile_specs_from_z(stack,z_q,render=r)    
    ...
    for matchobj in allmatches:
        ...
        try:
            ts_p = next(ts for ts in tilespecs_p if ts.tileId == matchobj['pId'])
            ts_q = next(ts for ts in tilespecs_q if ts.tileId == matchobj['qId'])
        except:
            try:
                ts_p = next(ts for ts in tilespecs_p if ts.tileId == matchobj['qId'])
                ts_q = next(ts for ts in tilespecs_q if ts.tileId == matchobj['pId'])
    ...
    xy=ColumnDataSource({'x0':all_points_global_p[:,0],
                         ...
                         'dv':dv})
    colors = [
        "#%02x%02x%02x" % (int(r), int(g), int(b)) for r, g, b, _ in 255*mpl.cm.cool(mpl.colors.Normalize(vmin=0,vmax=max_d)(dv))
    ]
    ...
    plot.segment('x0', 'y0', 'x1', 'y1', source=xy, line_width=1,line_color=colors)
...
@app.route('/rendersource/<path:rendersource>/owner/<owner>/project/<project>/stack/<stack>/collection/<collection>/pointmatch')
def pointmatch_summary_view(rendersource,owner,project,stack,collection):
    host,port = get_host_port(rendersource)
    render=renderapi.render.connect(host,port,owner,project,client_scripts="")
    collections = renderapi.pointmatch.get_matchcollections(render=render)
    collectionData = [c for c in collections if c['collectionId']['name']==collection]
...
@app.route('/rendersource/<path:rendersource>/owner/<owner>/project/<project>/stack/<stack>/collection/<collection>')
...
@app.route('/rendersource/<path:rendersource>/owner/<owner>/project/<project>/stack/<stack>')
...
def project_view(rendersource,owner,project):
    ...
    stackInfo = [s for s in renderapi.render.get_stack_metadata_by_owner(render=render) if s['stackId']['project']==project]
...
def stackInfo_to_df(stackInfo,rendersource):
    ...
    df['stack']=df.apply(lambda x: "<a href='/rendersource/%s/owner/%s/project/%s/stack/%s'> %s </a>"%\
                            (rendersource,x['owner'],x['project'],x['stack'],x['stack']),
                            axis=1)
    df['project']=df.apply(lambda x: "<a href='/rendersource/%s/owner/%s/project/%s'> %s </a>"%\
                            (rendersource,x['owner'],x['project'],x['project']),
                            axis=1)
    df['owner']=df.apply(lambda x: "<a href='/rendersource/%s/owner/%s'> %s </a>"%\
...
def rendersource_index(rendersource):
    ...
    return render_template('render.html', rendersource=rendersource,owners=owners)


@app.route('/rendersource/<path:rendersource>/owner/<owner>/project/<project>/stack/<stack>/collection/<collection>/pointmatch/z1/<int:z1>/z2/<int:z2>')
def pointmatch_z1_z2(rendersource,owner,project,stack,collection,z1,z2):
    try:
        ...
        return r.message
    return "host:%s port:%d rendersource:%s owner:%s project:%s stack:%s collection:%s"%(host,port,rendersource,owner,project,stack,collection)
